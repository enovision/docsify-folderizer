export class Folderizer {
    ...
    constructor(config, html) {
        ...
        me.validator = [me.charFile, me.charMap, me.charMapOpen, me.charPath];
    ...
    renderDiv() {
        ...
        if (!me.empty(me.config.wrapClass)) {
            divElement.classList.add(me.config.wrapClass);
        }
     ...
     */
    getContent(elText) {
        let me = this, elContent, elTextNode, type,
            pos = 0, ix, output,
            splitClass, splitCls,
            splitText = me.strsplit(elText, 1),
            validator = me.array_merge(me.validator, [' ']);

        for (ix = 0; ix < splitText.length; ix++) {
            if (!me.in_array(splitText[ix], validator)) {
                break;
            }
            pos++;
        }
    ...
    getType(elText) {
        ...
        for (var ix = 0; ix < splitText.length; ix++) {
            ...
            if (!me.in_array(text, me.validator)) {
                break;
            }

            if (text === me.charFile) {
                type = me.typeFile;
            } else if (text === me.charMap) {
                type = me.typeMap;
            } else if (text === me.charMapOpen) {
                type = me.typeMapOpen;
            }
        }
